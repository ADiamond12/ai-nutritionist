...
The model uses a scikit-learn MLP regressor with hidden layers `(32, 16)`, ReLU activation, Adam optimization, seeded `random_state=42`, and early stopping. The trained pipeline is cached in-process with a single retained entry so repeated recommendations are reproducible for the same code, data, and inputs without keeping multiple fitted pipelines in memory.
