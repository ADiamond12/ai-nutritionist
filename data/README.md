...
`recipes/` is a small reviewed ingredient-level Mediterranean pilot. It stores recipe, ingredient, and source CSVs, then runtime loading projects reviewed recipe rows back into the same flat catalog schema used by the recommender. Ingredient-level grocery output is available only for selected recipe-backed pilot rows.
